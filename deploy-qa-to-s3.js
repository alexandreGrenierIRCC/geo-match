...
const UploadFile = (branhcName, path, client) =>
  new Promise((resolve, reject) => {
    const fileObject = fs.readFileSync(
      `./dist/geo-match/${path[2]}`
    );
    console.log(`./dist/geo-match/${path[2]}`);
    // resolve(fileObject);
    const upload = new Upload({
      client: client,
      params: {
        Bucket: BUCKET_NAME,
        Key: `${branhcName}/${path[2]}`,
        Body: fileObject,
    ...
    upload.done().then((res, error) => {
      resolve(`Uploaded: ./dist/geo-match/${path[1]}`);
    });
  });
...
const editBaseHref = (branhcName) =>
  new Promise((resolve, reject) => {
    let file = cheerio.load(
      fs.readFileSync("./dist/geo-match/index.html")
    );
...
(async () => {
  ...
  for await (const f of getFiles("./dist/geo-match")) {
    const path = f.split("geo-match/");
